<script lang='ts'>
  ...
  async function changeFilter() {
    let input:string = `{\"accountId\": {\"_eq\": ${Number($page.params.accountId)}}`
    if (statusTypes.includes(statusFilter)) {
      input += `, \"status\": {\"_eq\": \"${statusFilter}\"}`
    } 
    if (dateFilter) {
      input += `, \"transactionDate\": {\"_gte\": \"${dateFilter}\"}`
    } 
    if (categoryFilter) {
      input += `, \"category\": {\"_like\": \"${categoryFilter.toLowerCase()}\"}`
    }
    input+="}"
    console.log(input)
...
<div class="">
  <select bind:value={statusFilter} on:change={() => changeFilter()}
  ...
  <input type='date' bind:value={dateFilter} on:change={() => changeFilter()}
    class="h-10 p-2 rounded-lg outline outline-1 outline-gray-400 shadow-sm" placeholder="Filter by Transaction Date"
  />
  <input bind:value={categoryFilter} on:change={() => changeFilter()}
