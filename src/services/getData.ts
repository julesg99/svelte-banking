import type { Order_By, AccountsFragment, GetAccountsQuery, GetFilteredTransactionQuery, TransactionsFragment, Transactions_Bool_Exp } from "../graphql/graphql"
...
export async function getFilteredTransactionsWithAggregates(where: Transactions_Bool_Exp, limit: number, offset: number, orderBy?: Order_By) {
  ...
  const response = await graphqlGetFilteredTransactionsWithAggregates({where: where, limit: limit, offset: offset, amountOrder: orderBy})
  if (response.errors) {
